...
<!-- TODO: These Task 1 example PDFs still show the OLD approval form. Each has a "\*" and there's a matching "\*" note under the list. As you regenerate each example from the new 4-box form, swap that example's link and drop its "\*"; remove the note once all are updated. -->
...
\* *These Task 1 examples use the **old** topic approval form. Task 1 now uses the new 4-box approval form; see [Submitting the Approval Form](task1:approval).*
